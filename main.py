...
if __name__ == "__main__":
    ...
    binfile = open("output1.mid", 'wb')
    midi.writeFile(binfile)
    binfile.close()
    # open_file('output1.mid')
    # 使用例
    input_file = 'output1.mid'
    output_file = 'output2.mid'
    semitones = 2
    transpose_midi(input_file, output_file, semitones)
    
    us = environment.UserSettings()
    us['lilypondPath'] = 'path_to_Your_lilypond'

    midi_file = 'output2.mid'
    score = converter.parse(midi_file)
    score_metadata = metadata.Metadata()
    score_metadata.title = 'Fire Emblem Theme transposition'
    score.metadata = score_metadata

    musicxml_file = 'output.musicxml'
